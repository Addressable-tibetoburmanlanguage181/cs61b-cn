...
/**
 * TETile 对象用于表示世界中的单个瓦片。瓦片的二维数组构成一个棋盘，
 * 可以使用 TERenderer 类将其绘制到屏幕上。
 *
 * 所有 TETile 对象必须有一个字符、文本颜色和背景颜色，用于在绘制到屏幕上时
 * 表示该瓦片。你还可以选择提供一个合适大小（16x16）的图片文件路径，
 * 用于替代 unicode 表示进行绘制。如果提供的图片路径找不到，draw 方法会
 * 回退到使用提供的字符和颜色表示，因此你可以自由地在自己的电脑上使用图片瓦片。
 *
 * 提供的 TETile 是不可变的，即其实例变量不能改变。如果你愿意，欢迎你
 * 将你的 TETile 类改为可变的。
 */
