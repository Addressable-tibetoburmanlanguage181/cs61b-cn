...
public class StringUtils {
    ...
    /**
     * 如果字符串 S 仅由 'a' 到 'z' 之间的字符组成，则返回 true。
    ...
    /**
     * 返回按字母顺序紧跟在 S 之后的字符串。
     * 例如，如果 s 是 'potato'，此方法将返回 'potatp'。如果
     * 最后一个字符是 z，则我们添加到下一个位置，依此类推。
     */
    public static String nextString(String s) {
        /* 处理所有 zs 作为特殊情况以保持辅助方法简单。 */
        if (isAllzs(s)) {
    ...
    /**
     * nextString 的辅助函数。将 X 的第 P 个位置递增一，
     * 如果 p == 'z'，则回绕到 'a'。如果发生回绕，
     * 则我们需要进位，并递增位置 P - 1。
     *
     * 对于仅包含 zs 的字符数组将失败。
     */
    ...
    /** 如果 S 全是 'z'，则返回 true。空字符串返回 false */
