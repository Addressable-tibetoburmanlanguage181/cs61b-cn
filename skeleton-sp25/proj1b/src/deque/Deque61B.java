...
public interface Deque61B<T> {
    ...
    /**
     * 将 {@code x} 添加到双端队列的前端。假定 {@code x} 不为 null。
     *
     * @param x 要添加的元素
     */
    void addFirst(T x);
    ...
    /**
     * 将 {@code x} 添加到双端队列的后端。假定 {@code x} 不为 null。
     *
     * @param x 要添加的元素
     */
    void addLast(T x);
     ...
     * 但我们添加了这个额外操作以提供更多编程练习。
     * 通过迭代方式获取元素。如果索引越界则返回 null。
     * 不会改变原双端队列。
     *
     * @param index 要获取的索引
    ...
    /**
     * 严格来说这个方法不应该在接口中，但为了方便测试而放在这里。
     * 通过递归方式获取元素。如果索引越界则返回 null。
     * 不会改变原双端队列。
     *
     * @param index 要获取的索引
     * @return 双端队列中 {@code index} 位置的元素
